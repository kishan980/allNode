...
const TransferAPI_1 = require("./TransferAPI");
describe('TransferAPI', () => {
    ...
    describe('getTransfers', () => {
        it('returns withdrawals', () => __awaiter(void 0, void 0, void 0, function* () {
            const response = [mockWithdrawal];
            nock_1.default(global.REST_URL).get(TransferAPI_1.TransferAPI.URL.TRANSFERS).query(true).reply(200, response);
            const withdrawals = yield global.client.rest.transfer.getTransfers(TransferAPI_1.TransferType.WITHDRAW);
            expect(withdrawals.data.length).toBe(1);
            expect(withdrawals.data[0].id).toBe('6b09bf5e-c94c-405b-b7dc-ad2b27749ce5');
        }));
        it('returns deposits', () => __awaiter(void 0, void 0, void 0, function* () {
            const response = [mockDeposit];
            nock_1.default(global.REST_URL).get(TransferAPI_1.TransferAPI.URL.TRANSFERS).query(true).reply(200, response);
            const deposits = yield global.client.rest.transfer.getTransfers(TransferAPI_1.TransferType.DEPOSIT, 'test-profile-id');
            expect(deposits.data.length).toBe(1);
            expect(deposits.data[0].id).toBe('6cca6a14-a5e3-4219-9542-86123fc9d6c3');
        }));
    });
    describe('getTranfer', () => {
        it('returns a withdrawal', () => __awaiter(void 0, void 0, void 0, function* () {
            const response = mockWithdrawal;
            const withdrawalId = '6b09bf5e-c94c-405b-b7dc-ad2b27749ce5';
            const path = `${TransferAPI_1.TransferAPI.URL.TRANSFERS}/${withdrawalId}`;
            nock_1.default(global.REST_URL).get(path).reply(200, response);
            const withdrawal = yield global.client.rest.transfer.getTransfer(withdrawalId);
            expect(withdrawal.id).toBe(withdrawalId);
        }));
    });
});
//# sourceMappingURL=TransferAPI.test.js.map